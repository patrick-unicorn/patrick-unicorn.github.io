...
本篇与前一篇一样，是一篇经典的论文[《The Lambda Calculus - A Brief Introduction》][0], 仍然是为了深入理解Scheme相关范畴的内容，所附带学习的基础性知识。第一次接触Lambda是在C#语言中，当时的第一印象是一种“匿名函数”，除此之外，对其背后的数学思想也并未深究。再到后来，逐渐接触到了[函数式编程][1]概念，了解到了其背后的数学思想就是[Lambda算子][2]，并进行了初步的了解，但也是不求甚解。这次在翻看Scheme相关的资料（书籍、论文等）时，感觉是时候深入的理解下其数学意义了。本文是一篇导论性质的文章，有很强的前后依赖性，因此不能跳读，只能按序阅读章节，并需要具备一定的数学基础(高中程度就可以了)和某种语言中的Lambda使用经验(如Java8/C#/Scheme等等)，才能比较好的理解。另外由于文中有很多数理概念的英语单词，为了保证正确性(中文翻译出来总感觉挺别扭的)，我都进行了保留。下面是译文（不断修正完善中）：
...
在许多编程语言中，函数内部调用自身(即递归调用)可以简单地使用函数名。这被称为隐式递归，但是这点再Lambda算子中是不能实现的，因为Lambda算子中所有的**lambda abstraction**均为匿名函数。
...
> **译注：关于**不动点**以及**Y-Combinator**的详细说明可参考[《康托尔、哥德尔、图灵——永恒的金色对角线(rev#2)》][4]。
...
> (Y M) →<sup>β</sup> ((λx.(M(x x)))(λx.(M(x x))))
        ...
        →<sup>β</sup> (M(Y M))
>
> **译注：**第一步中，直接用Y的定义替代`Y`，并应用β-reduction变换，此时Y(`λf.((λx.(f(x x))))(λx.(f(x x)))`)中的绑定变量`f`全部代换为`M`；第二步中，第一个`λx.(M(x x))`作为β-reduction定义中的`λx.M`，第一个`λx.(M(x x))`作为β-reduction定义中的`N`；第三步中，对(λx.(M(x x)))(λx.(M(x x)))应用β-abstraction。
...
> 如何使用Lambda算子实现`+`?
> 
> 最简单直观的方式，把`+`当做一个**两参函数**: `λx.λy.x+y` <=> `λxy.x+y`，问题是显而易见，函数体中的`+`引用了自身；为了解决消除这一递归定义矛盾，很自然我们会想到Y-Combinator，因此Y-Combinator就是解决Lambda算子中递归问题而引入的。下面是具体的过程：
> 
> `λx.λy.x+y` => `λxy.x y` => `
> 
