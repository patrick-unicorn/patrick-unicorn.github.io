...
**{:}** Scheme中的赋值表达式为`set! [var-name] [var-value]`，很多情况下，其可以被lambda过程(let本质上也是Lambda过程)所替代。
...
**{:}** let绑定的一般形式`(let [name] ((var expr) ...) body1 body2...)`，`let`的含义可以理解为**定义一个过程并用一组参数调用该过程**。需要注意三点：
...
**{:}** `call/cc`用于获取**当前continuation**，并将其作为另一个函数f传入参数，同时调用f。[注：在R5RS标准中，实际定义的过程名为`call-with-current-continuation`，但是很多scheme环境中都可简化为`call/cc`]
...
+ 如果没有在f内部没有调用continuation，那么f本身的返回值即为call/cc的返回值。
