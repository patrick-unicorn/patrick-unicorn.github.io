---
title: "A Little Understanding of Type System"
...
就硬件层面而言，通常仅仅支持一组数学概念上的最小类型集合：整数、实数、布尔、字符。而在软件层面，不同的程序设计语言会提供必要的扩展类型和扩展机制(允许程序员自定义类型，如OOP中的class)。

事实上，以集合论的视角观察类型系统，会对很多类型的相关概念有更加清晰的理解。类型就是一个值域(集合)，而类型系统则是类型集合，如下：
...
对于程序语言而言，类型系统是必须的么？这取决于我们观察的角度。就语法层面而言，类型系统并非必须；但在程序执行层面，类型系统是必须的。因为计算机的本质是数值计算，因此，在执行层面至少存在数值类型。
...
+ [Covariance_and_contravariance](http://en.wikipedia.org/wiki/Covariance_and_contravariance_(computer_science))
